...
public class ReservationServiceTest extends ProtocolTest {
    ...
    public void init() {
        ...
        system1.setTypes(Set.of(
                createReservationType(0, "Quickbuchung", 3, 60, 0, UserRole.REGISTERED, UserRole.TRAINER, UserRole.KIOSK, UserRole.TECHNICAL, UserRole.TEAMSTER),
        ...
        var system2 = new SystemConfigEntity();
        system2.setId(2L);
        ...
        system2.setTypes(Set.of(
                createReservationType(0, "Quickbuchung", 1, 14, 8, UserRole.REGISTERED, UserRole.TRAINER, UserRole.KIOSK, UserRole.TECHNICAL, UserRole.TEAMSTER, UserRole.GUEST),
                createReservationType(1, "Training", UserRole.TRAINER, UserRole.TEAMSTER),
                createReservationType(2, "Meisterschaft", UserRole.TRAINER, UserRole.TEAMSTER),
                createReservationType(3, "Dauerbuchung)"),
                createReservationType(4, "Gesperrt", UserRole.TRAINER),
                createReservationType(5, "Jugendtraining", 1, 1, 0, UserRole.REGISTERED, UserRole.TEAMSTER)));
        system2.setNew(true);
        systemConfigRepository.save(system2);
        saveTypes(system2);

        var system3 = new SystemConfigEntity();
        system3.setId(3L);
    ...
    public void addReservationWithWrongUser() throws Exception {
        addReservationError(createReservation(1, user, 1, 10, 2),
    ...
    public void addReservationNotOverlapDifferentDays() throws Exception {
        var reservation = createReservation(1, 1, 11, 2);
    ...
    public void addReservationNoText() throws Exception {
        var reservation = createReservation(1, 1, 8, 2);
    ...
    public void addReservationBeforeOpeningHour() throws Exception {
        ...
        addReservationWithOccupationFieldError(reservation,
                "start", "Startzeit 6:00 liegt vor der Öffnungszeit 8:00");
    }

    @Test
    @WithMockUser(username = "TRAINER")
    public void addReservationAfterClosingHour() throws Exception {
        var reservation = createReservation(1, 1, 23, 2);
        addReservationWithOccupationFieldError(reservation,
                "start", "Startzeit 23:00 liegt später als die Endezeit 22:00");
    }
    ...
    public void addReservationInvalidDurationUnits() throws Exception {
        ...
        addReservationWithOccupationFieldError(reservation,
                "start", "fehlerhafte Startzeit mit 25 Minuten");
    }
    ...
    public void addReservationExtendsClosingHour() throws Exception {
        ...
        addReservationWithOccupationFieldError(reservation,
                "start", "Startzeit + Dauer zu spät");
    }

    @Test
    @WithMockUser(username = "TRAINER")
    public void addReservationDurationTooSmall() throws Exception {
        var reservation = createReservation(1, 1, 8, 0);
    ...
    public void addReservationDurationEvenTooBigForAdmin() throws Exception {
        ...
        reservation.setDate(reservation.getDate().plusDays(1));
        addReservationWithOccupationFieldError(reservation,
                "duration", "Der Benutzer TRAINER hat nicht die Rechte, eine Reservierung der Dauer 4 anzulegen");
    }
    ...
    public void addReservationTooManyCourts() throws Exception {
        ...
        addReservationWithFieldError(reservation,
                "court", "Platznummer 7 zu groß. Mehr als 6 Plätze gibt es nicht");
    }
    ...
    private void checkReservationMultipleCourts(int start, int expectedOccupations, int... courts) throws Exception {
        var reservation = createReservation(1, 1, start, 1, 1);
        reservation.setCourtsFromInteger(courts);
        var savedReservation = getReservation(addReservation(reservation));
        var occupations = occupationRepository.findByReservationId(savedReservation.getId());
    ...
    private Reservation checkReservationRepeatDaily(int hour, int repeatDays, int expectedOccupations, int... courts) throws Exception {
        var reservation = createReservation(1, 1, hour, 2);
    ...
    private void checkReservationRepeatWeekly(int hour, int repeatDays, int expectedOccupations, int... courts) throws Exception {
        var reservation = createReservation(1, 1, hour, 2);
        reservation.setRepeatUntil(reservation.getDate().plusDays(repeatDays));
    ...
    public void addReservationNoRepeatUntil() throws Exception {
        var reservation = createReservation(1, 3, 12, 2);
        reservation.setRepeatType(RepeatType.daily);
        addReservationWithFieldError(reservation,
                "repeatUntil", "Kein Datum Wiederholung bis angegeben");
    }
    ...
    public void addReservationRepeatUntilBeforeStart() throws Exception {
        ...
        addReservationWithFieldError(reservation,
                "repeatUntil", "Datum Wiederholung bis muss hinter dem Reservierungsdatum liegen");
    }
    ...
    public void addReservationNoSystemConfig() throws Exception {
        addReservationError(createReservation(0, 1, 10, 6),
                HttpStatus.NOT_FOUND,
                "SYSTEM_CONFIGURATION mit id 0 nicht gefunden");
    }
    ...
    public void addReservationUnauthorizedTimeInThePast() throws Exception {
        ...
        var systemConfig = getSystemConfig(1);
    ...
    public void addReservationUnauthorizedInTheFarFuture() throws Exception {
        ...
        addReservationWithOccupationFieldError(reservation,
                "date",
                "Reservierungen dürfen maximal 60 Tage im Voraus eingetragen werden");
    }
    ...
    public void addReservationUnauthorizedTypePrepaid() throws Exception {
        ...
        addReservationWithOccupationFieldError(reservation, //
                "type",
                "Der Benutzer REGISTERED hat nicht die Rechte, eine Reservierung vom Typ Dauerbuchung anzulegen");
    }
    ...
    public void addReservationUnauthorizedTypeTraining() throws Exception {
        ...
        addReservationWithOccupationFieldError(reservation, //
                "type",
                "Der Benutzer REGISTERED hat nicht die Rechte, eine Reservierung vom Typ Training anzulegen");
    }

    @Test
    @WithMockUser(username = "TRAINER")
    public void updateOccupation() throws Exception {
        var reservation = getResponseJson(addReservation(createReservation(2, 1, 12, 2, 1)), Reservation.class);
        var occupation = reservation.getOccupations().getFirst();
    ...
    public void updateOccupations() throws Exception {
        var reservation = checkReservationRepeatDaily(8, 3, 4, 2);
    ...
    public void deleteReservation() throws Exception {
        var reservation = objectMapper
                .readValue(addReservation(createReservation(1, 1, 10, 2))
                        .andExpect(status().isCreated())
                        .andReturn().getResponse().getContentAsString(), Reservation.class);
    ...
    public void getOccupationsWithDate() throws Exception {
        var reservation = getReservation(addReservation(createReservation(2, 2, 12, 1)));
        var epochMilli = LocalDate
                .now()
                .plusDays(1)
                .atStartOfDay()
    ...
    public void getReservation() throws Exception {
        var reservation = reservationRepository.save(ReservationMapper.map(
                createReservation(1, user, 1, 10, 2)));

        performGet("/rest/reservation/id/" + reservation.getId()).andExpect(status().isOk());
    }
    ...
    private ReservationSystemConfig getSystemConfig(long id) throws Exception {
        var content = performGet("/rest/reservation/systemconfig/" + id)
    ...
    private ResultActions addReservation(Reservation reservation) throws Exception {
        return checkReservation(addReservationNoCheck(reservation)
                .andExpect(status().isCreated()), reservation, ActionType.CREATE);
    }

    private ResultActions addReservationError(Reservation reservation, HttpStatus status, String message) throws Exception {
        return checkError(addReservationNoCheck(reservation), status, message);
    }

    private void addReservationWithOccupationFieldError(Reservation reservation, String... fieldErrors) throws Exception {
    ...
    private void addReservationWithFieldError(Reservation reservation, String... fieldErrors) throws Exception {
        var resultActions = addReservationError(reservation, HttpStatus.BAD_REQUEST, null);
        var i = 0;
        while (i < fieldErrors.length) {
    ...
    private ResultActions checkReservation(ResultActions resultActions, Reservation reservation, ActionType actionType) throws Exception {
        switch (actionType) {
            case CREATE -> {
                resultActions.andExpect(status().isCreated())
                        .andExpect(jsonPath("$.id").isNotEmpty())
                        .andExpect(jsonPath("$.type").value(reservation.getType()));
                var createdReservation = getResponseJson(resultActions, Reservation.class);
    ...
    private void checkOccupation(ResultActions resultActions, Occupation occupation, ActionType actionType) throws Exception {
        switch (actionType) {
            case CREATE:
                resultActions.andExpect(status().isCreated())
                        .andExpect(jsonPath("$.id").isNotEmpty())
    ...
    private UserEntity createUser(UserRole role, ActivationStatus status) {
        var name = role.toString() + "." + status.toString();
    ...
    private String roles(UserRole... roles) {
        return Stream.concat(Stream.of(UserRole.ADMIN), Arrays.stream(roles)).map(Enum::toString).collect(Collectors.joining(","));
    }
}
